...
- 敵を倒すと、マスかダイスの報酬を1つ選びます。マス報酬はマスとマスの間（列は上/右/下/左）にある
  「＋」マーカーをタップで選びます。1回目のタップで実際にどのマスが入るかをその場でプレビューし、
  同じ場所をもう一度タップすると確定します。既存のマスは壊れません。
  1列あたり5マスまで、盤の合計は16マスまで。片方でも達した列は間のマーカーごと消え、挿し込めなくなり、
...
- `test_growth.gd`: マス報酬によるボード成長（マスとマスの間のマーカーをタップ、1マス刻み、
  列ごとに上限5・盤全体で上限16）が指定した列だけを伸ばすこと、既存のマスを一枚も失わず順序も
  保つこと、マスをタップしてもプレビューは始まらないこと、上限に達した列には間のマーカー自体が
  出ないこと、ボード全体としては他の列でまだ伸ばせること、両方の上限に達すると置き換えに戻ること、
  非対称に伸びた盤でも四隅ワープが解決することを確認します。四隅のマスのすぐ後ろの継ぎ目（上→右、
  右→下、下→左）だけは、その隅マス自身の列を伸ばすのではなく、次の列の先頭に挿し込む扱いにして
  あること（そうしないと新しいマスが隅の位置を奪い、既存の隅マスが弾き出されて見えてしまうため）
  も確認します。
