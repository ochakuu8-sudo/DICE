...
- 戦闘に入ると、キャラ固有の8マスすごろくボード（3x3の外周）で戦います。空きマスは無く、8マス全部に効果があります。
...
- 敵を倒すと、マスかダイスの報酬を1つ選びます。マス報酬はボードの外周（5x5＝16マスが上限）を2マス伸ばし、
  選んだマスが2枚、既存のマスを壊さずに加わります。上限に達すると、以後のマス報酬は既存のマスを選んで置き換えます。
...
- `test_growth.gd`: マス報酬によるボード成長（8→16マスまで2マス刻み）が既存のマスを一枚も失わず、
  順序も保ったまま伸びること、上限到達後は置き換えに戻ること、四隅ワープが伸びた盤でも解決することを確認します。
