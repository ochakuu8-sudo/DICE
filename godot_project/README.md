...
- 戦闘に入ると、キャラ固有の8マスすごろくボードで戦います。空きマスは無く、8マス全部に効果があります。
  ...
  盤全体の物理サイズと四隅の位置は常に固定で、マスの大きさは現在いちばん詰まっている列に
  合わせて自動的に縮みます（過密で重なることはありません）。
...
- 敵を倒すと、マスかダイスの報酬を1つ選びます。マス報酬は挿し込む場所をタップで選び、
  ...
  1列あたり5マスまで、盤の合計は16マスまで。片方でも達した列は挿し込めなくなり（タップしても反応しません）、
  すべての列が上限に達すると以後のマス報酬は既存のマスを選んで置き換えます。
...
- `test_growth.gd`: マス報酬によるボード成長（1マス刻み、列ごとに上限5・盤全体で上限16）が
  タップした列だけを伸ばすこと、既存のマスを一枚も失わず順序も保つこと、上限に達した列は
  タップしても伸びずボード全体としては他の列でまだ伸ばせること、両方の上限に達すると
  置き換えに戻ること、非対称に伸びた盤でも四隅ワープが解決することを確認します。
