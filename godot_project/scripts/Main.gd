...
# square. Sixteen is the size the very first version of a fully-tiled
# board was checked against and read fine on screen.
const MAX_RING_LEN := 16
...
func _build_ui() -> void:
	...
	# A tap anywhere over the board and hand throws the dice that have not
	# been thrown yet. It sits above the cells, so while anything in hand is
	# still face down the whole play area is one big "roll" button, and it
	# disappears the moment the last of them lands.
...
func _refresh_place_panel() -> void:
	if place_panel == null:
		return
	var placing: bool = (state == "reward_place" or state == "reward_insert") \
		and pending_reward_type != "" and tile_defs.has(pending_reward_type)
	place_panel.visible = placing
	if not placing:
		_place_card_type = ""
		return
	var cache_key := "%s:%s" % [state, pending_reward_type]
	if _place_card_type == cache_key:
		return
	_place_card_type = cache_key
	_clear_children(place_body)

	var heading := _make_label(FS_HEAD, COL_TEXT, HORIZONTAL_ALIGNMENT_CENTER, true)
	heading.text = tr("挿し込むマス") if state == "reward_insert" else tr("置くマス")
...
func _layout_board_buttons() -> void:
	...
	if banner != null and zone_board != null:
		...
		# once.
		var interior: float = max(_board_spacing() * float(ring_extent.x - 2) - _board_token_size() - 16.0, 120.0)
...
func _layout_ribbon() -> void:
	...
	var center := _board_origin() + Vector2(float(ring_extent.x - 1), float(ring_extent.y - 1)) * 0.5 * step
...
func _refresh_command() -> void:
	...
	if roll_catcher != null:
		roll_catcher.visible = playing and not _unthrown_dice().is_empty()
	...
	# Nothing to reroll until the hand has been thrown in the first place.
	reroll_button.disabled = not (playing and rerolls_left > 0 and not hand.is_empty() \
		and _unthrown_dice().is_empty())
...
func _refresh_board() -> void:
	...
	for y in range(MAX_BOARD_H):
		for x in range(MAX_BOARD_W):
			...
			var is_player_cell := pos == player_pos and state != "reward_place" and state != "reward_insert"
...
func _refresh_ribbon() -> void:
	if ribbon_box == null:
		return
	var interior: float = _board_spacing() * float(ring_extent.x - 2) - _board_token_size() - 12.0
	...
	ribbon_box.visible = (state == "player" and _unthrown_dice().is_empty()) or moving
...
func _make_die_card(die: Dictionary, index: int, slot_w: float = 120.0, slot_h: float = 140.0) -> Control:
	...
	# The result, big. Before this die has been thrown it is a question mark:
	# the card is a die you have not thrown yet, not a die with no value —
	# which matters, because 居合 can legitimately roll a real 0.
	...
	var unrolled: bool = not bool(die.get("thrown", true))
	var is_warp: bool = not unrolled and _is_warp_face(roll)
	...
	if unrolled or is_warp:
		...
		# The corner labels are two characters (左上/右上/...) where "?" and
		# 帰 are one, so the font has to shrink to still fit the face.
	...
	for face_value in sorted_faces:
		...
		pip.mouse_filter = Control.MOUSE_FILTER_IGNORE
		pip.modulate = Color(1, 1, 1, 1.0 if (not unrolled and fv == roll) else 0.55)
...
func _start_run(key: String) -> void:
	...
	side_len = {"top": 3, "right": 2, "bottom": 2, "left": 1}
	_build_track_graph()
...
func _start_encounter() -> void:
	...
	if encounter == 1:
		intro = "画面をタップして伏せたダイスを振り、出た目から1つ選んで進みます。使わなかったダイスは目のまま次のターンへ残ります。"
...
var enemy_defs := [
	...
	{"name": "贄喰い", "art": "captain", "hp": 52, "damage": 9, "kind": "cell",
		"mode": "relative", "cells": [2, 3, 4, 5], "thorns": 2, "gold": 25,
		"debuff": "burn", "foul": 50, "part": "chest"},
	{"name": "深淵の主", "art": "boss", "hp": 62, "damage": 10, "kind": "guaranteed",
...
func _load_run() -> bool:
	...
	var starting_len := {"top": 3, "right": 2, "bottom": 2, "left": 1}
	var saved_len: Dictionary = data.get("side_len", {})
	side_len = {}
	var total := 0
	for side in SIDE_NAMES:
	...
	# fall back to the starting board rather than build something this
	# session never actually produced.
	if total > MAX_RING_LEN:
		side_len = starting_len.duplicate()
	_build_track_graph()
	part_dev = {"chest": 0, "depths": 0, "tail": 0}
...
# Fills the hand back up to its limit. A die arrives face down — "thrown"
# false, and a roll of 0 that nothing is allowed to read — and the tap is
# what turns it over. A die carried in from last turn is already thrown and
# keeps the face it was showing, so the hand at the start of a turn is a
# mix: the results the player chose to keep, and question marks over the
# ones just drawn.
#
# throw_now is for the 集中路-style refill in the middle of a turn. The rest
# of the hand is face up by then, so a die drawn there is thrown on arrival
# rather than waiting for a tap that has already happened.
func _draw_to_hand(throw_now: bool) -> void:
	while hand.size() < hand_limit:
		...
		drawn["thrown"] = throw_now
		drawn["roll"] = _random_face(drawn) if throw_now else 0
		hand.append(drawn)

# Which dice in hand are still face down. Empty means the hand is readable
# and the turn can be planned; anything in it means the tap is still owed.
func _unthrown_dice() -> Array:
	var out := []
	for i in range(hand.size()):
		if not bool((hand[i] as Dictionary).get("thrown", true)):
			out.append(i)
	return out
...
# The turn's opening throw: everything still face down goes over at once,
# and a die carried in from last turn is not touched — keeping it is what
# not spending it bought. Free, and owed once per turn; the reroll below is
# the one that costs something.
func _throw_unthrown() -> void:
	if state != "player":
		return
	var pending := _unthrown_dice()
	if pending.is_empty():
		return
	_hide_banner()
	for i in pending:
...
func _on_die_pressed(index: int) -> void:
	if state != "player" or index < 0 or index >= hand.size() or actions_left <= 0:
		return
	# A hand with anything still face down cannot be planned against, so the
	# tap that lands on a card throws the hand rather than asking about one
	# die of it. The roll catcher covers the play area and normally takes
	# this tap first; this is the same answer for the taps it does not.
	if not _unthrown_dice().is_empty():
		await _throw_unthrown()
		return
...
func _previewed_die() -> Dictionary:
	if state != "player" or not _unthrown_dice().is_empty():
		return {}
	...
	match op:
		...
		"draw":
			...
			for i in range(amount):
				hand_limit += 1
			_draw_to_hand(_unthrown_dice().is_empty())
			for i in range(amount):
				hand_limit -= 1
			_refresh_hand()
...
# Tap-to-preview, tap-again-to-confirm, same two-stage pattern as
...
func _confirm_insert(pos: Vector2i) -> void:
	...
	_set_log(tr("%s を%s列に挿し込んだ（全%dマス）。") % [
		pending_reward_name, SIDE_NAME_JA[side], ring_cells.size()])
	_snap_player_visual()
	_refresh_all()
	_show_map()
...
func _on_cell_pressed(index: int) -> void:
	...
	# (_idx, MAX_BOARD_W) — the buttons themselves never move or get
...
# --- ring growth ---------------------------------------------------------
# Placing a tile reward grows one side of the board by exactly one square
# instead of overwriting a square already on it, so a run never spends
...
func _side_of(pos: Vector2i) -> String:
	var idx: int = int(ring_index_map.get(pos, -1))
	if idx < 0 or idx >= ring_side.size():
		return "top"
...
# it. The ring's grid shape is only ever a rendering detail —
# _build_track_graph regenerates it fresh from side_len — so this does
# not try to track which physical grid cell held which tile across the
# resize: it reads the current content off in ring order, splices the one
# new entry in at the chosen point, rebuilds the (now one square longer)
# grid shape, and writes the same ordered content back onto it. A tile
...
func _grow_ring_with_tile(tile_id: String, after_pos: Vector2i) -> bool:
	...
	content.insert(insert_index, tile_id)
	fouled.insert(insert_index, "none")
	...
	if old_step >= insert_index:
		player_step = old_step + 1

	side_len[side] = int(side_len[side]) + 1
	_build_track_graph()
...
# A corner is the last square of the side that runs into it (top's own
# last square is the top-right corner; the next side, right, starts one
# past it) — not a fixed ring step, since which square that is depends on
# every side_len grown before it, so this reads it off the live ring
# rather than off WARP_FACES. RESET_FACE and the top-left corner always
# land on the start, which is ring_cells[0] regardless of size.
func _warp_face_step(v: int) -> int:
	if v == RESET_FACE or v == CORNER_TL:
		return 0
	var side := ""
	match v:
		CORNER_TR:
		...
		_:
			return 0
	# The square right before that side begins is the corner it turns at.
	var idx := ring_side.find(side)
